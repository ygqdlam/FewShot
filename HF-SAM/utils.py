...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0 and gt.sum() > 0:
        ...
        specific=metric.binary.recall(pred,gt)
        precision=metric.binary.precision(pred,gt)
        return dice, hd95,jaccard,asd,recall,specific,precision
...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                outputs1,_= net(inputs, multimask_output, patch_size[0], gt=None, mode='test')
                # if stage == 3:
    ...
    metric_list_asd=[]
    metric_list_recall=[]
    metric_list_specific=[]
    metric_list_precision=[]
    for i in range(1, classes + 1):
        ...
        metric_list_jaccard.append(calculate_metric_percase(prediction == i, label == i)[2])
        metric_list_asd.append(calculate_metric_percase(prediction == i, label == i)[3])
        metric_list_recall.append(calculate_metric_percase(prediction == i, label == i)[4])
        metric_list_specific.append(calculate_metric_percase(prediction == i, label == i)[5])
        metric_list_precision.append(calculate_metric_percase(prediction == i, label == i)[6])
