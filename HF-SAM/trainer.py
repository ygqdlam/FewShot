...
class AddGaussianNoise(object):
    def __init__(self, mean=0.0, variance=1.0, amplitude=1.0, p=1.0):
        self.mean = mean
    ...
    def __call__(self, img):
        ...
        arr = np.asarray(img)  # (H,W) or (H,W,C)
        # 灰度图 -> (H,W,1)
        if arr.ndim == 2:
            arr = arr[..., None]
        H, W, C = arr.shape

        noise = self.amplitude * np.random.normal(
...
class AddPepperNoise(object):
    ...
    椒盐噪声
    Args:
        snr (float): signal-to-noise ratio，保留原像素的比例 (0,1]
        p (float): 施加噪声的概率
    """
    def __init__(self, snr: float, p: float = 0.9):
        if not (0 < snr <= 1):
            raise ValueError(f"snr should be in (0,1], got {snr}")
        if not (0 <= p <= 1):
            raise ValueError(f"p should be in [0,1], got {p}")
        self.snr = float(snr)
        self.p = float(p)
...
def trainer_acdc(args, model, snapshot_path, multimask_output, low_res):
